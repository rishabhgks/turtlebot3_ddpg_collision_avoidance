    ...
    def turtlebot_is_crashed(self, laser_values, range_limit):
        ...
        for i in range(len(laser_values)):
            ...
            if (laser_values[i] < range_limit):
                ...
                print("Reset by Laser Crash, Dist: {}".format(laser_values[i]))
                self.reset()
                time.sleep(1)
                break
        ...
        for i in range(comp_depth_image.shape[0]):
            for j in range(comp_depth_image.shape[1]):
                if comp_depth_image[i][j] == 0.0:
                    comp_depth_image[i][j] = np.inf
        flat_depth = comp_depth_image.max(axis=0)
        scaled_depth = [min(flat_depth[i:i+8])/2.0 for i in np.arange(0, depth_image.shape[1], 8)]
